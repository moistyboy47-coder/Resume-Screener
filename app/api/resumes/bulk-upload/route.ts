...
import { uploadResumeToStorage, saveCandidateEvaluations, getJobPosting } from "@/lib/supabase/db";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const jobId = (formData.get("job_id") as string) || "a1b2c3d4-e5f6-7a8b-9c0d-1e2f3a4b5c6d";
    ...
    const processSingleFile = async (file: File): Promise<CandidateEvaluation | null> => {
      try {
        ...
        let resumeUrl = "";
        try {
          resumeUrl = await uploadResumeToStorage(buffer, file.name, file.type || "application/pdf");
        } catch (storageErr) {
          console.warn(`Storage upload warning for ${file.name}:`, storageErr);
        }

        // Run AI Evaluation
        const evaluation = await evaluateResume(resumeText, job!, resumeUrl, model);
        return evaluation;
      } catch (err) {
    ...
    const rawResults = await processBatchThrottled(files, 2, 2000, processSingleFile);
    const validEvaluations = rawResults.filter((item): item is CandidateEvaluation => item !== null);
    ...
    try {
      ...
      console.warn("Database insert warning (returning evaluations anyway):", dbErr);
    }
